...
import { addAppoinment } from "../../helper/supabase"
import dayjs from "dayjs"


export default function AppoinmentCard({doctorData, appoinmentData, user}) {
    ...
    const handleClick = async ()=> {
        await addAppoinment({setloading: setloading, router: router, data: user, id: router.query?.appoinmentId})
    }
    ...
    return (
        <div className="bg-white rounded-2xl">
            ...
            <div className="flex flex-col justify-center items-center space-y-6 py-4">
                ...
                <button className="submit px-10" onClick={handleClick}>
                    {loading ? 'Loading...' : 'Confirm'}
                </button>

                <p className="cursor-pointer underline" onClick={()=>router.push('/user/doctor/'+ doctorData?.id)}>Go to back</p>
            </div>
